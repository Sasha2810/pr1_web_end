...
function glav() {
    ...
    del.addEventListener("click", () => { main.push("del") })
...
function switch_active() {
    maths_numbers.forEach(n => n.addEventListener('click', () => {
        ...
        // Добавление
        elem = document.querySelector(`.${clas}`);
        console.log(elem)
...
console.log(level)
if (level.innerText == 'Level1 Math') {
    first_page();
}

if (level.innerText == 'Level2 Math') {
    second_page();
}

if (level.innerText == 'Level3 Math') {
    third_page();
}

function first_page() {
   answers =  ["3", "12", "12", "10", "можно", "15"];
   return answers;
}

function second_page() {
    answers =  ["45", "спичку", "6", "124416", "черепаха", "6"];
    return answers;
}

function third_page() {
    answers =  ["верно", "спичку", "6", "124416", "45", "6"];
    return answers;
}
